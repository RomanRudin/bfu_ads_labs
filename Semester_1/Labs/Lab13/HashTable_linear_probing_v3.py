...
class HashTable:
    ...
    def __iter__(self)-> Generator[Any, Any, None]:
        yield from [key for key, state in zip(self.table, self.state) if state == 1] 
    ...
    def __eq__(self, other: "HashTable") -> bool:
        ...
        if type(self) is not type(other):
            return False
        return set([key for key, state in zip(self.table, self.state) if state == 1]) == set([key for key, state in zip(other.table, other.state) if state == 1]) 

    def __str__(self) -> LiteralString:
        pairs = []
        for index, key in enumerate(self.table):
            if self.state[index] == 0: continue
